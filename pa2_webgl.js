...
var currentProgram;
var lightProgram;
//  Final project Control Point for curve
var pointProgram; 
...
function initShaders() {
    ...
    // Initializing control point drawing shader
    pointProgram = createShaderProg("shader-vs-light", "shader-fs-control-point");
    pointProgram.vertexPositionAttribute = gl.getAttribLocation(pointProgram, "aVertexPosition");
    gl.enableVertexAttribArray(pointProgram.vertexPositionAttribute);
    pointProgram.pMatrixUniform = gl.getUniformLocation(pointProgram, "uPMatrix");
}
...
// Create Buffer for bezierPoint
var bezierPointBuffer;
function initBuffers() {
    lightPositionBuffer = gl.createBuffer();
    bezierPointBuffer = gl.createBuffer();
}
...
var pointMatrix = mat4.create();

var bezier_points = new Array();
bezier_points[0] = vec3.create(); bezier_points[0].set([1.5, 0.0, 0.0]);
bezier_points[1] = vec3.create(); bezier_points[1].set([1.5, -0.8, 0.0]);
bezier_points[2] = vec3.create(); bezier_points[2].set([-1.5, 0.8, 0.0]);
bezier_points[3] = vec3.create(); bezier_points[3].set([-1.5, 0.0, 0.0]);
bezier_points[4] = vec3.create(); bezier_points[4].set([-1.5, -0.8, 0.0]);
bezier_points[5] = vec3.create(); bezier_points[5].set([0.0, -0.8, 0.0]);
bezier_points[6] = vec3.create(); bezier_points[6].set([0.0, 0.0, 0.0]);
bezier_points[7] = vec3.create(); bezier_points[7].set([0.0, 0.8, 0.0]);
bezier_points[8] = vec3.create(); bezier_points[8].set([1.5, 0.8, 0.0]);
bezier_points[9] = vec3.create(); bezier_points[9].set([1.5, 0.0, 0.0]);

var obj_size = 1.0;

var cp = vec3.create();
var inter_point = vec3.create();
...
function drawScene() {
    ...
    mat4.translate(mvMatrix, inter_point);
    ...
    // Bezier animate
    if ( bezier_animated ) {
        mat4.identity(pointMatrix);
        mat4.translate(pointMatrix, [0.0, 0.0, -3.0]);

        // setup points
        gl.useProgram(pointProgram);
        gl.uniformMatrix4fv(pointProgram.pMatrixUniform, false, pMatrix);
        gl.bindBuffer(gl.ARRAY_BUFFER, bezierPointBuffer);
        for (var i = 0; i < bezier_points.length - 1; i ++) {
            cp.set(bezier_points[i]);
            mat4.multiplyVec3(pointMatrix, cp);
            gl.bufferData(gl.ARRAY_BUFFER, Float32Array.from(cp), gl.DYNAMIC_DRAW);
            gl.vertexAttribPointer(pointProgram.vertexPositionAttribute, 3, gl.FLOAT, false, 0, 0);
            gl.drawArrays(gl.POINTS, 0, 1);
        }
...
function tick() {
    ...
    if ( lastTime != 0 ) {
      ...
      // trace the curve  
      if ( bezier_animated ) {
        inter_point.set(getInterpolationPoint(bezier_points, t));

        if (t + elapsed/2000.0 > 1.0)
            t = 0.0;
        else
            t += elapsed/2000.0;
...
function getInterpolationPoint(points, t) {
    if (points.length == 1)
        return points[0];
    ...
    for (var i = 1; i < points.length; i++) {
        var p = vec3.create();
        p = vec3.lerp(points[i - 1], points[i], t, p);
        newPoints.push(p);
    }

    return getInterpolationPoint(newPoints, t);
}
